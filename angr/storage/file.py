...
class SimFileStream(SimFile):
    ...
    def __init__(self, name, pos=0, **kwargs):
        super(SimFileStream, self).__init__(name, **kwargs)
        self._pos = pos

    def set_state(self, state):
        super(SimFileStream, self).set_state(state)
        if type(self.pos) in (int, long):
            self._pos = state.solver.BVV(self.pos, state.arch.bits)

    def read(self, _, size, **kwargs):
        data, size, self.pos = super(SimFileStream, self).read(self.pos, size, **kwargs)
...
class SimPacketsStream(SimPackets):
    ...
    def read(self, _, size, **kwargs):
        data, size, self.pos = super(SimPacketsStream, self).read(self.pos, size, **kwargs)
        return data, size, None
