...
from ..storage.file import SimFileStream
...
class SimCGC(SimUserland):
    ...
    def state_tracer(self, magic_content=None, preconstrain_flag=True, input_content=None, **kwargs):
        ...
        state = self.state_full_init(mode='tracing', stdin=SimFileStream, flag_page=magic_content, **kwargs)
        state.preconstrainer.preconstrain_file(input_content, state.posix.stdin, set_length=True)

        #if preconstrain_flag and magic_content is not None:
            #state.preconstrainer.preconstrain_flag_page(magic_content)
        return state
